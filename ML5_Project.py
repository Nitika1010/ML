...
import pickle
import statsmodels.api as sm  # Added statsmodels import
...
with tab4:
    ...
    if not model_df.empty and len(features) > 0:
        ...
        try:
            ...
            st.success("Scikit-learn model successfully trained and saved.")
        except Exception as e:
            st.error(f"Error saving scikit-learn model: {e}")
        ...
        st.write(f"Scikit-learn Model R-squared: {r2:.2f}")
        st.write(f"Scikit-learn Mean Squared Error: {mse:.2e}")

        # Statsmodels model (with more detailed statistics)
        st.subheader("Statsmodels OLS Regression")
        ...
        st.write(f"Statsmodels R-squared: {r2_sm:.2f}")
        st.write(f"Statsmodels Mean Squared Error: {mse_sm:.2e}")

        # Actual vs Predicted plot (both models)
        results_df = pd.DataFrame({
            'Actual': y_test,
            'Predicted (Scikit-learn)': y_pred,
            'Predicted (Statsmodels)': y_pred_sm
        }).sample(min(500, len(y_test)), random_state=42)

        fig = px.scatter(
            results_df,
            x='Actual',
            y=['Predicted (Scikit-learn)', 'Predicted (Statsmodels)'],
            trendline="lowess",
            title='Actual vs Predicted Earnings (Both Models)',
            template='plotly_white'
        )
        fig.add_shape(
            type="line",
        ...
        coef_df_sk = pd.DataFrame({
            'Feature': ['Intercept'] + features,
            'Coefficient (Scikit-learn)': np.concatenate([[model.intercept_], model.coef_])
        })
        
        # Statsmodels coefficients
        coef_df_sm = pd.DataFrame({
            'Feature': sm_model.params.index,
            'Coefficient (Statsmodels)': sm_model.params.values,
            'P-value': sm_model.pvalues
        })
        
        # Merge both
        coef_df = pd.merge(coef_df_sk, coef_df_sm, on='Feature')
        
        # Display top 20 features by absolute coefficient value
        coef_df['Max_Abs_Coeff'] = coef_df[['Coefficient (Scikit-learn)', 'Coefficient (Statsmodels)']].abs().max(axis=1)
        top_coef_df = coef_df.sort_values('Max_Abs_Coeff', ascending=False).head(20)
        
        fig_coef = px.bar(
            top_coef_df,
            x='Feature',
            y=['Coefficient (Scikit-learn)', 'Coefficient (Statsmodels)'],
            barmode='group',
            title='Top 20 Feature Coefficients Comparison',
            template='plotly_white'
        )
        ...
        # Add p-values table for statsmodels
        st.subheader("Statsmodels P-values")
        st.dataframe(coef_df_sm.sort_values('P-value').style.format({'P-value': '{:.4f}'}))
...
- Both scikit-learn and statsmodels provide effective earnings prediction
- Statsmodels provides additional statistical significance information
""")